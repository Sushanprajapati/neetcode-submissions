    ...
    public void put(int key, int value) {
        ...
        if (node != null) {
            ...
            if (hashMap.size() == cache_capacity) {
                // Here, the order matters because, our map is referencing the address of node.
                // We first have to remove that connection from map
                // We know that LRU cache will always be prev to tail.
                hashMap.remove(tail.prev.key);
                // Finally, remove the node
                removeNode(tail.prev);
            }

            // Now, we'll have to add the node next to head (Making it last visited)
            // Initialize the node with new values
