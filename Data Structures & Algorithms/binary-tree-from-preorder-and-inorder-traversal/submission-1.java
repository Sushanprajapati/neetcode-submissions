...
class Solution {
    ...
    private TreeNode splitTree(int[] preorder, Map<Integer, Integer> inOrderMap, int rootIndex, int left, int right) {
        ...
        // we'll also narrow down the left and right window, just like we did for binary search

        // move the root index to one step ahead for left child's root
        if (mid > left) {
            root.left = splitTree(preorder, inOrderMap, rootIndex + 1, left, mid - 1);
        }

        // if mid index < right, we know right tree exists
        // narrow down the new tree's left and right index
        // calculate the rootIndex using the new formula. this is because, there could be 
        // possibility of null values as well, which means no child node.
        if (mid < right) {
