class Solution {
    ...
    public int leastInterval(char[] tasks, int n) {
        ...
        while (!pq.isEmpty()) {
            ...
            // we'll increment the time with temList size if queue is already empty when we reach here
            // this is because, once the queue is empty, we know that all required values will be there in tempList
            // so, simply taking its size, we know the time.
            if (pq.isEmpty()) {
                time += tempList.size(); 
            } else {
                // if queue is still not empty, just like we did above, out time will increase by n+1 (same constraint)
                // we know there will be n+1 elements for one cycle
                time += n+1;
            }
        }
        return time;
