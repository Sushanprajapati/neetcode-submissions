class Solution {
    ...
    public void backTracking(List<List<Integer>> resultSet, List<Integer> tempSet, int[] nums, int start) {
        ...
        for (int i = start; i < nums.length; i++) {
            ...
            // It's called once the loop fails 
            // We're removing the latest/last element from list
            // it'll then keep on removing the stacks in call stacks. basically, will
            // put only the candidate that we chose in the call stack. its all happening
            // in the background
